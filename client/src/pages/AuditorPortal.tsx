...
const AuditorPortal = () => {
  ...
  useCrossPortalSync({
    onRefreshRequired: (source: string, reason: string) => {
      console.log(
        `AuditorPortal: Refreshing data due to: ${reason} (from: ${source})`,
      );
      fetchAuditData();
    },
  ...
  const fetchAuditData = async () => {
    try {
      ...
      const totalBalance =
        balances?.reduce((sum, b) => sum + Number(b.current_balance), 0) || 0;
      const netPosition =
        totalContributions - totalDisbursements - totalExpenses;
      const memberCount = members?.length || 0;
      const activeMembers =
        members?.filter((m) => m.maturity_status === "mature").length || 0;
      const averageContribution =
        memberCount > 0 ? totalContributions / memberCount : 0;
      ...
      const riskScore = Math.max(
        0,
        100 - riskFactors.reduce((sum, factor) => sum + factor, 0),
      );
      ...
      const discrepanciesFound = [
        ...(balances?.filter(
          (b) =>
            ...
            Number(b.total_contributions) - Number(b.total_disbursements),
        ) || []),
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8 p-4">
          ...
          <div
            className={`${healthStatus.bgColor} ${healthStatus.borderColor} border-2 rounded-lg p-4`}
          >
        ...
        <div className="grid gap-4 md:grid-cols-4 mb-8 px-4">
          ...
          <Card className="bg-gradient-to-br from-blue-50 to-cyan-50 border-blue-200">
            ...
            <CardContent>
              ...
              <p className="text-xs text-blue-600">
                {(
                  (metrics.totalDisbursements / metrics.totalContributions) *
                  100
                ).toFixed(1)}
                % of contributions
              </p>
          ...
          <Card className="bg-gradient-to-br from-purple-50 to-violet-50 border-purple-200">
            ...
            <CardContent>
              <div
                className={`text-2xl font-bold ${metrics.netPosition >= 0 ? "text-green-900" : "text-red-900"}`}
              >
                KES {metrics.netPosition.toLocaleString()}
              </div>
              <p className="text-xs text-purple-600">
                After expenses: KES{" "}
                {(metrics.netPosition - metrics.totalExpenses).toLocaleString()}
              </p>
            </CardContent>
          </Card>
