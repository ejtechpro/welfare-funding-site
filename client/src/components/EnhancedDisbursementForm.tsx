...
import { generateBereavementPDF } from "@/docs/pdf/BereavementPDF";
...
export const EnhancedDisbursementForm = ({
  onSuccess,
}: {
  onSuccess?: () => void;
}) => {
  ...
  const fetchMembers = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select(
          "id, user.firstName, user?.lastName, tnsNumber, phone, email, address",
        )
        .eq("registration_status", "approved")
        .order("user.firstName", { ascending: true });
  ...
  const fetchDisbursements = async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        if (docsError) {
          if (
            docsError.message?.includes("does not exist") ||
            docsError.message?.includes("schema cache")
          ) {
            console.warn(
              "Document storage table not found. Document features will be limited.",
            );
    ...
    try {
      ...
      if (checkError) {
        ...
        if (
          ...
          throw new Error(
            "Database table not found. Please contact your administrator to set up the document storage table.",
          );
        }
        throw checkError;
      }
      ...
      const uploadedBy = staffUser
        ? `${staffUser.user.firstName} ${staffUser.user?.lastName} (${staffUser.staff_role})`
        : "Unknown";
      ...
      if (error.message?.includes("Database table not found")) {
        errorMessage =
          "Document storage not set up. Please ask your administrator to run the database setup script.";
        ...
        errorMessage =
          "Unsupported file type. Please upload PDF, DOC, DOCX, JPG, or PNG files.";
      } else if (
        error.message?.includes("does not exist") ||
        error.message?.includes("schema cache")
      ) {
        errorMessage =
          "Document storage table not found. Please contact your administrator.";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data: disbursementData, error: disbursementError } =
        await supabase
          .from("disbursements")
          .insert({
      ...
      if (currentBalance) {
        await supabase
          ...
          .update({
            ...
            total_disbursements:
              Number(currentBalance.total_disbursements) + numAmount,
        ...
        <TabsContent value="new-disbursement">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Record a disbursement payment to a member and generate
                bereavement forms if needed
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    >
                      ...
                      <SelectContent>
                        {members.data?.map((member) => (
                          <SelectItem
                            key={member.id}
                            value={String(member.id)} // 🔥 important
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="disbursementType">
                      Disbursement Type *
                    </Label>
                    <Select
        ...
        <TabsContent value="manage-documents">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Upload completed bereavement forms and manage disbursement
                documents
              </CardDescription>
            </CardHeader>
            <CardContent>
                                      ...
                                      <div className="flex flex-col">
                                        ...
                                        {databaseDoc && (
                                          <span className="text-xs text-gray-500">
                                            {databaseDoc.filename} •{" "}
                                            {(
                                              databaseDoc.file_size /
                                              1024 /
                                              1024
                                            ).toFixed(2)}{" "}
                                            MB
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() =>
                ...
                handleFileUpload(selectedDisbursementId)
              }
